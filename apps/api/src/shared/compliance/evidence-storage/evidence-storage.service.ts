...
export class EvidenceStorageService {
  ...
  async storeEvidenceWithIntegrity(evidence: any): Promise<StoredEvidence> {
    try {
      ...
      this.logger.log(`Evidence stored with integrity: ${evidenceHash.substring(0, 16)}...`);
      ...
      this.logger.error(`Failed to store evidence: ${error.message}`, error.stack);
      throw error;
    }
  }

  async verifyEvidenceChain(): Promise<{ valid: boolean; issues: string[]; chainLength: number }> {
    const issues: string[] = [];
    ...
    try {
      ...
      for (let i = 0; i < chainEntries.length; i++) {
        ...
        if (entry.previousHash !== previousHash) {
          issues.push(`Hash mismatch at position ${i}: expected ${previousHash}, got ${entry.previousHash}`);
          valid = false;
        }
        
        // Verify current hash
        const calculatedHash = this.generateHash(JSON.stringify(entry.evidenceData));
        if (entry.evidenceHash !== calculatedHash) {
          issues.push(`Hash verification failed at position ${i}: stored ${entry.evidenceHash}, calculated ${calculatedHash}`);
          valid = false;
        }
  ...
  async cleanupOldEvidence(retentionDays: number = 365): Promise<{ deletedCount: number }> {
    try {
      const cutoffDate = new Date();
      cutoffDate.setDate(cutoffDate.getDate() - retentionDays);
      
      // Delete old evidence collections
      const deletedCollections = await this.prisma.evidenceCollection.deleteMany({
        where: {
          collectedAt: { lt: cutoffDate },
        },
      });
      
      this.logger.log(`Cleaned up ${deletedCollections.count} old evidence collections`);
