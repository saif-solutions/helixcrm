  ...
  router.stack.forEach(layer => {
    ...
    if (layer.name === 'router' && layer.handle && layer.handle.stack) {
      if (Array.isArray(layer.handle.stack)) {
        layer.handle.stack.forEach(nestedLayer => {
          if (nestedLayer.route && nestedLayer.route.path && nestedLayer.route.stack && nestedLayer.route.stack[0]) {
            const route = {
    ...
    if (analyticsRoutes.length === 0) {
      ...
      expectedEndpoints.forEach(expected => {
        const found = analyticsRoutes.find(route => 
      ...
      // Check HTTP methods
      const hasOnlyGetMethods = analyticsRoutes.every(route => 
        route.method === 'GET'
      );
      console.log(`✅ HTTP methods: ${hasOnlyGetMethods ? 'All GET (correct)' : 'Mixed methods found'}`);
      ...
      console.log(`✅ Duplicates: ${duplicates.length === 0 ? 'None found' : `${duplicates.length} duplicates detected`}`);
