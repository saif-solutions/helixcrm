...
export class Soc2ControlsService {
  ...
  private readonly CONTROL_MAPPING = {
    ...
    'CC6.1': {
      ...
      criteria: 'Security',
      description: 'Implement logical access security software, infrastructure, and architectures to protect information assets.',
    },
    'CC6.2': {
  ...
  async verifyControl(
    ...
    try {
      ...
      if (evidence) {
        ...
        } else if (typeof evidence.data === 'object' && Object.keys(evidence.data).length > 0) {
          evidenceCount = 1;
          status = 'PASS';
      ...
      this.logger.log(`Control ${controlId} verified: ${status} (${evidenceCount} evidence items)`);
      ...
      this.logger.error(`Failed to verify control ${controlId}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async getControlVerificationHistory(
    ...
    try {
      ...
      this.logger.error(`Failed to get control verification history: ${error.message}`);
      throw error;
    }
  }

  async getControlStatusSummary(days: number = 30): Promise<Record<string, any>> {
    try {
      ...
      for (const [criteria, info] of Object.entries(this.CONTROL_MAPPING)) {
        ...
        const controlStatus = recentControls.find(v => v.controlId === criteria);
        summary.byCriteria[criteriaKey].total++;
        summary.byCriteria[criteriaKey][controlStatus?.status.toLowerCase() || 'unknown']++;
      ...
      this.logger.error(`Failed to get control status summary: ${error.message}`);
      throw error;
    }
  }

  private calculateOverallStatus(verifications: any[]): 'HEALTHY' | 'WARNING' | 'CRITICAL' {
    const statusCounts = {
      PASS: 0,
      FAIL: 0,
      PARTIAL: 0,
  ...
  async getControlDetails(controlId: string): Promise<any> {
    ...
    const recentVerifications = await this.getControlVerificationHistory(controlId, undefined, undefined, undefined, 10);
  ...
  private getImplementationStatus(controlId: string): 'IMPLEMENTED' | 'PARTIAL' | 'PLANNED' {
    // This should be enhanced to check actual implementation
    const implementedControls = ['CC6.1', 'CC6.2', 'CC6.6', 'A1.1', 'A1.2', 'C1.1', 'PI1.1', 'P1.1'];
