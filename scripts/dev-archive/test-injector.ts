...
async function testInjector() {
  ...
  try {
    ...
    const { Soc2EvidenceService } =
      await import('./src/shared/compliance/soc2/soc2-evidence.service');
    console.log('✓ Soc2EvidenceService class loaded');

    // Test EvidenceStorageService
    const { EvidenceStorageService } =
      await import('./src/shared/compliance/evidence-storage/evidence-storage.service');
    console.log('✓ EvidenceStorageService class loaded');

    // Test Soc2ControlsService
    const { Soc2ControlsService } =
      await import('./src/shared/compliance/soc2/soc2-controls.service');
