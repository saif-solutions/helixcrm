...
import type { DataGridProps as DataGridPropsType, DataGridRef } from './DataGrid.types';

// Import utility functions from types file
import { getCellValue, getRowId, dataGridAria } from './DataGrid.types';
...
const DataGridInner = React.forwardRef<DataGridRef, DataGridPropsType<any>>((props, ref) => {
  const {
    data,
    columns,
    rowKey = 'id',
  ...
  const renderHeader = () => (
    <thead className={cn(dataGridClasses.header.container, headerClassName)}>
      <tr>
        {selectable && (
          <th
            className={getHeaderCellClasses({ align: 'left', sticky: true }, 'bg-gray-50')}
            scope="col"
          >
        ...
        {actions && (
          <th className={getHeaderCellClasses({ align: 'left' }, 'bg-gray-50')} scope="col">
            Actions
          </th>
        )}
      </tr>
          ...
          >
              ...
              return (
                <td key={`${rowId}-${column.key}`} className={cellClasses}>
                  {column.render ? (
                    ...
                    <div className={cn(column.align === 'right' && 'text-right')}>{cellValue}</div>
                  )}
                </td>
              );
            })}
            ...
            {actions && (
              <td
                ...
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-center gap-2">{actions(row, index)}</div>
              </td>
            )}
          </tr>
        );
        ...
        <div className="flex items-center gap-4">
          {bulkActions && currentSelected.length > 0 && (
            <div className={dataGridClasses.bulkActions}>
              <span className="text-sm text-gray-700">{currentSelected.length} selected</span>
              {bulkActions}
            </div>
          )}
