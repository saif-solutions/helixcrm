...
export class AnalyticsRepository extends TenantAwareRepository {
   ...
   */
  async getDealAnalyticsFromOperational(query: DealAnalyticsQueryDto): Promise<DealAnalyticsResponse> {
    try {
      ...
      this.logger.error(`Failed to get deal analytics: ${error.message}`, error.stack, {
   ...
   */
  async getRevenueAnalyticsFromOperational(query: RevenueAnalyticsQueryDto): Promise<RevenueAnalyticsResponse> {
    try {
      const tenantId = this.tenantId;
      const processedQuery = this.processDateRange(query);
      const where = this.buildDealWhereClause(tenantId, processedQuery);
      ...
      return {
        ...
        summary: {
          ...
          bestPerformingPipeline: await this.getBestPerformingPipeline(tenantId, processedQuery),
   ...
   */
  async getPipelineAnalyticsFromOperational(query: PipelineAnalyticsQueryDto): Promise<PipelineAnalyticsResponse> {
    try {
      const tenantId = this.tenantId;
      const processedQuery = this.processDateRange(query);
      ...
      const [pipelineData, bottleneckData, pipelineStats] = await Promise.all([
        ...
        query.includeBottlenecks ? this.getPipelineBottlenecks(tenantId, processedQuery) : Promise.resolve([]),
   ...
   */
  async getActivityAnalyticsFromOperational(query: ActivityAnalyticsQueryDto): Promise<ActivityAnalyticsResponse> {
    try {
      const tenantId = this.tenantId;
      const processedQuery = this.processDateRange(query);
      ...
      const where = {
        ...
        createdAt: {
          ...(processedQuery.startDate ? { gte: processedQuery.startDate } : {}),
...
const [activities, total, byType, userActivity] = await Promise.all([
  this.prisma.auditLog.findMany({
    where,
    include: {  // KEEP AS include SINCE actor RELATION EXISTS
      actor: {
        select: {
          id: true,
          email: true,
...
const recentActivities: ActivityRecord[] = activities.map(log => ({
  ...
  userName: log.actor?.firstName && log.actor?.lastName 
  ...
  metadata: log.metadata as Record<string, any> // Use metadata field
      ...
      const activityByType = byType.reduce((acc, item) => {
      ...
      this.logger.error(`Failed to get activity analytics: ${error.message}`, error.stack, {
        tenantId: this.tenantId,
        query
      });
   ...
   */
  async getAvailableExports(query: AnalyticsExportQueryDto, tenantId: string, userId: string): Promise<ExportJob[]> {
    try {
      const where = {
        organizationId: tenantId,
        userId,
      ...
      this.logger.error(`Failed to get available exports: ${error.message}`, error.stack, {
        tenantId,
        userId,
        query
      });
  ...
  private processDateRange(query: any): any {
    ...
    return {
      ...rest,
      startDate: startDate ? new Date(startDate) : new Date(Date.now() - this.defaultDateRangeDays * 24 * 60 * 60 * 1000),
  ...
  private async getDealsOverTime(tenantId: string, query: any): Promise<Array<{ date: string; count: number }>> {
  ...
  private async getStageMetrics(tenantId: string, query: any): Promise<Array<{ stage: string; count: number; value: number }>> {
    const deals = await this.prisma.deal.findMany({
      where: this.buildDealWhereClause(tenantId, query),
      select: {
        status: true,
  ...
  private async calculateSalesVelocity(tenantId: string, query: any): Promise<number> {
    ...
    const totalDays = deals.reduce((sum, deal) => {
      const days = Math.ceil((deal.closedAt!.getTime() - deal.createdAt!.getTime()) / (1000 * 60 * 60 * 24));
      return sum + days;
    }, 0);
  ...
  private async calculateForecastRevenue(tenantId: string, query: any): Promise<number> {
    const deals = await this.prisma.deal.findMany({
      where: {
        ...this.buildDealWhereClause(tenantId, query),
  ...
  private async getRevenueOverTime(tenantId: string, query: any): Promise<Array<{ date: string; revenue: number }>> {
    const deals = await this.prisma.deal.findMany({
      where: {
        ...this.buildDealWhereClause(tenantId, query),
        status: 'WON',
  ...
  private calculateGrowthRate(revenueOverTime: Array<{ date: string; revenue: number }>): number {
    if (revenueOverTime.length < 2) return 0;
    
    const sorted = [...revenueOverTime].sort((a, b) => a.date.localeCompare(b.date));
  ...
  private async getBestPerformingPipeline(tenantId: string, query: any): Promise<string | undefined> {
  ...
  private async getTopPerformer(tenantId: string, query: any): Promise<string | undefined> {
  ...
  private async getPipelineDataFromOperational(tenantId: string, query: any) {
    ...
    return pipelines.map(pipeline => {
      ...
      const activeDeals = allDeals.filter(d => this.ACTIVE_DEAL_STATUSES.includes(d.status));
  ...
  private calculateAverageStageDuration(deals: any[]): number {
    ...
    const durations = deals
      ...
      .map(deal => Math.ceil((deal.closedAt.getTime() - deal.createdAt.getTime()) / (1000 * 60 * 60 * 24)));
  ...
  private async getPipelineBottlenecks(tenantId: string, query: any) {
    const pipelineData = await this.getPipelineDataFromOperational(tenantId, query);
    
    const bottlenecks = [];
    for (const pipeline of pipelineData) {
      for (const stage of pipeline.stages) {
        if (stage.avgStageDuration > 30) { // More than 30 days is a bottleneck
          bottlenecks.push({
            stageId: stage.id,
            stageName: stage.name,
            averageDuration: stage.avgStageDuration,
...
private async getPipelineStatistics(tenantId: string, query: any) {
  ...
  const [totalDeals, totalValue, wonDeals, dealDurations] = await Promise.all([
    ...
    this.prisma.deal.findMany({
      where: { ...where, status: 'WON', createdAt: { not: null }, closedAt: { not: null } },
  ...
  if (dealDurations.length > 0) {
    const totalDays = dealDurations.reduce((sum, deal) => {
      return sum + Math.ceil((deal.closedAt!.getTime() - deal.createdAt!.getTime()) / (1000 * 60 * 60 * 24));
    }, 0);
  ...
  private async getUserActivityStats(tenantId: string, query: any) {
    ...
    const userIds = userActivity.map(item => item.actorUserId!).filter(Boolean);
    ...
    return userActivity.map(item => ({
      ...
      userName: userMap.get(item.actorUserId!)?.firstName && userMap.get(item.actorUserId!)?.lastName
      ...
      lastActive: item._max.createdAt?.toISOString() || new Date().toISOString()
    }));
  }
}