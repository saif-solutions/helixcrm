...
async function diagnose() {
  ...
  const modelProps = prismaProps.filter(prop => 
    prop === prop.toLowerCase() && 
    !prop.startsWith('$') &&
    !['_', 'transaction', 'interactiveTransaction', 'disconnect', 'connect', 'on', '$use'].includes(prop)
  );
  ...
  for (const model of phase3Models) {
    ...
    console.log(`   ${exists ? '✅' : '❌'} ${model}: ${exists ? 'FOUND' : 'NOT FOUND'}`);
  }
