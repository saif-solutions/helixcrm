...
vi.mock('./DialogSplitContext', () => {
  ...
  const applyShakeClass = () => {
    const el =
      document.querySelector('[data-testid="dialog"]') || document.querySelector('[role="dialog"]');
...
const getDialogByTestId = (testId: string = 'dialog') => screen.getByTestId(testId);
const queryDialog = () => {
  const dialog = screen.queryByRole('dialog');
  if (dialog) return dialog;
...
describe('Dialog - Variants and Sizes', () => {
  ...
  test.each([
    ...
    render(
      <Dialog open={true} onClose={mockOnClose} variant={variant as any}>
        Test Content
      </Dialog>,
    );
  ...
  test.each([
    ...
    render(
      <Dialog open={true} onClose={mockOnClose} size={size as any}>
        Test Content
      </Dialog>,
    );
...
describe('Dialog - Header', () => {
  ...
  test('renders header with icon', () => {
    ...
    render(
      <Dialog open={true} onClose={mockOnClose} title="Dialog Title" icon={icon}>
        Test Content
      </Dialog>,
    );
  ...
  test('hides close button when showCloseButton=false', () => {
    render(
      <Dialog open={true} onClose={mockOnClose} title="Dialog Title" showCloseButton={false}>
        Test Content
      </Dialog>,
    );
  ...
  test('close button renders (behavior tested in integration suite)', async () => {
    render(
      <Dialog open={true} onClose={vi.fn()} title="Dialog Title" showCloseButton={true}>
        Test Content
      </Dialog>,
    );
  ...
  test('close button requests close when clicked', async () => {
    ...
    render(
      <Dialog open={true} onClose={onClose} title="Dialog Title" showCloseButton persistent={false}>
        Test Content
      </Dialog>,
    );
  ...
  test('action buttons trigger onClick handlers', async () => {
    ...
    const customActions = [{ label: 'Test Action', onClick: mockActionClick }];

    render(
      <Dialog open={true} onClose={vi.fn()} footer={{ actions: customActions }}>
        Test Content
      </Dialog>,
    );
...
describe('Dialog - Body and Content', () => {
  ...
  test('body has proper accessibility attributes', () => {
    render(
      <Dialog open={true} onClose={mockOnClose} title="Dialog Title">
        Test Content
      </Dialog>,
    );
...
describe('Dialog - Footer and Actions', () => {
  ...
  test('renders custom actions', () => {
    ...
    render(
      <Dialog open={true} onClose={mockOnClose} footer={{ actions: customActions }}>
        Test Content
      </Dialog>,
    );
  ...
  test('disabled action buttons are not clickable', async () => {
    ...
    const customActions = [{ label: 'Disabled Action', disabled: true, onClick: mockActionClick }];
...
describe('Dialog - User Interactions', () => {
  ...
  test('does not call onClose when Escape is pressed and closeOnEscape=false', async () => {
    render(
      <Dialog open={true} onClose={mockOnClose} closeOnEscape={false}>
        Test Content
      </Dialog>,
    );
  ...
  test('does not call onClose when overlay is clicked and closeOnOverlayClick=false', async () => {
    ...
    render(
      <Dialog open={true} onClose={mockOnClose} closeOnOverlayClick={false}>
        Test Content
      </Dialog>,
    );
...
describe('Dialog - Focus Management', () => {
  ...
  test('focus trap wraps backward with Shift+Tab', async () => {
    render(
      <Dialog open={true} onClose={vi.fn()} footer={false}>
        {' '}
        {/* ← ADD footer={false} */}
        <button>First</button>
  ...
  test('returns focus to the triggering element on close', async () => {
    const TestApp = () => {
      ...
      return (
        <>
          <button ref={triggerRef} id="trigger" onClick={() => setOpen(true)}>
            Open Dialog
          </button>
          <Dialog open={open} onClose={() => setOpen(false)} returnFocusRef={triggerRef}>
...
describe('Dialog - State and Animation', () => {
  ...
  test('calls onOpenComplete when dialog opens', () => {
    ...
    render(
      <Dialog open={true} onClose={vi.fn()} onOpenComplete={mockOnOpenComplete}>
        Test Content
      </Dialog>,
    );
...
describe('Dialog - Persistent Behavior', () => {
  ...
  test('persistent dialog blocks escape key', async () => {
    const onClose = vi.fn();
    render(
      <Dialog open persistent onClose={onClose} title="Test">
        Content
      </Dialog>,
    );
    await userEvent.keyboard('{Escape}');
  ...
  test('persistent dialog blocks close button', async () => {
    const onClose = vi.fn();
    render(
      <Dialog open persistent onClose={onClose} title="Test" showCloseButton>
        Content
      </Dialog>,
    );
...
describe('Dialog - Performance', () => {
  ...
  describe('Dialog - Performance Regression', () => {
    ...
    test(`renders within ${PERFORMANCE_THRESHOLD}ms`, () => {
      ...
      console.log(
        `Dialog render time: ${renderTime.toFixed(2)}ms (threshold: ${PERFORMANCE_THRESHOLD}ms)`,
      );
    });
    ...
    test('stress test: multiple dialog toggles', () => {
      ...
      console.log(
        `Multiple dialog toggle time: ${totalTime.toFixed(2)}ms (threshold: ${STRESS_THRESHOLD}ms)`,
      );
    });
  });
});
...
describe('Dialog - Accessibility (WCAG 2.1 AA)', () => {
  ...
  test('dialog with form elements is accessible', async () => {
    await testAccessibility(
      <Dialog open={true} onClose={vi.fn()} variant="form" title="User Settings">
  ...
  test('dialog without title provides fallback accessibility', async () => {
    render(
      <Dialog open={true} onClose={vi.fn()} ariaLabel="Image preview dialog">
  ...
  test('nested dialog maintains proper accessibility context', async () => {
    await testAccessibility(
      <Dialog open={true} onClose={vi.fn()} title="Parent Dialog" nested={true} nestedLevel={1}>
