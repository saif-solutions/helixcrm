...
export class ConfigValidationService {
  ...
  async validate(): Promise<ValidationResult> {
    ...
    this.logger.log('í´ Starting configuration validation...');
    ...
    try {
      ...
      this.logger.log('âœ… Database connection validated');
    } catch (error) {
    ...
    // Step 4: Log results
    if (errors.length > 0) {
      this.logger.error(`âŒ Configuration validation failed with ${errors.length} error(s):`);
      errors.forEach(error => this.logger.error(`  - ${error}`));
    } else {
      this.logger.log(`âœ… Configuration validation passed`);
    }

    if (warnings.length > 0) {
      this.logger.warn(`âš ï¸  Configuration validation has ${warnings.length} warning(s):`);
