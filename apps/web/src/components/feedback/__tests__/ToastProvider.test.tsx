...
const TestComponent = () => {
  ...
  return (
    <div>
      ...
      <button onClick={() => toast.error('Error!', 'Something went wrong')}>Show Error</button>
      <button onClick={() => toast.info('Info', 'Here is some information')}>Show Info</button>
      <button onClick={() => toast.warning('Warning', 'This is a warning')}>Show Warning</button>
    </div>
  );
};
