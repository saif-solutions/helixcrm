...
describe('Dialog - Integration Tests (Real Implementation)', () => {
  ...
  test('close button calls onClose when clicked (real implementation)', async () => {
    ...
    render(
      <Dialog open={true} onClose={onClose} title="Test Dialog" showCloseButton={true}>
        Content
      </Dialog>,
    );
  ...
  test('focus trap works with real implementation', async () => {
    ...
    const buttons = screen
      .getAllByRole('button')
      .filter((btn) => ['First', 'Second', 'Third'].includes(btn.textContent || ''));
  ...
  test('autoFocus works correctly', async () => {
    ...
    render(
      <Dialog open={true} onClose={onClose} title="AutoFocus Test" initialFocusRef={inputRef}>
