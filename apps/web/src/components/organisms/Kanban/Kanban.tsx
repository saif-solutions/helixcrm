...
const getProbabilityBadgeClass = (probability?: number): string => {
  if (probability === undefined) return 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300';
  if (probability >= 80) return 'bg-success-100 text-success-800 dark:bg-success-900 dark:text-success-200';
  if (probability >= 50) return 'bg-warning-100 text-warning-800 dark:bg-warning-900 dark:text-warning-200';
  ...
  JSON.stringify({
    ...
    columnId: column.id,
      ...
      const handleCardDrop = React.useCallback((
        ...
        if (readOnly || dragState.type !== 'card' || !dragState.draggingCard || !dragState.sourceColumn) {
          return;
        }
        ...
        // If moving between columns
          ...
          const sourceCards = sourceColumn.cards.filter((c: KanbanCard) => c.id !== draggingCard.id);
...
event.dataTransfer.setData(
  ...
  column.id
      ...
      // Handle add card
      const handleAddCard = React.useCallback((column: KanbanColumn) => {
        if (readOnly) return;
      ...
      // Handle delete card
      const handleDeleteCard = React.useCallback((card: KanbanCard, column: KanbanColumn) => {
        if (readOnly) return;
      ...
      const handleDeleteColumn = React.useCallback((column: KanbanColumn) => {
        if (readOnly) return;
      ...
      const calculatePipelineValue = React.useCallback(() => {
        return columns.reduce((total, column) => {
          return total + column.cards.reduce((colTotal, card) => {
...
const contextValue: KanbanContextValue = React.useMemo(() => ({
  ...
  columns, readOnly, allowColumnReorder, allowCardReorder, allowCardMoveBetweenColumns,
  dragState, activeCard, activeColumn, handleCardDragStart, handleCardDragEnd, 
  handleColumnDragStart, handleColumnDrop, handleCardDrop
]);

      // Render default card
      const renderDefaultCard = React.useCallback((card: KanbanCard, column: KanbanColumn) => {
  ...
  if (e.key === 'Enter' || e.key === ' ') {
    ...
    handleCardClick(card, column, e as any);
          ...
          >
            ...
            {/* Card Body */}
            {card.description && (
              <div className={kanbanClasses.card.body}>
                {card.description}
              </div>
            )}
...
{showProbability && card.probability !== undefined && (
  ...
  {(card.storyPoints || card.estimatedHours || card.actualHours) && (
    <div className="flex items-center gap-2">
      {card.storyPoints && (
        <span data-testid={`${testId}-card-sp-${card.id}`} className="flex items-center gap-1">
          <span className="font-medium">SP:</span>{card.storyPoints}
        </span>
      )}
      {card.estimatedHours && (
        <span data-testid={`${testId}-card-est-${card.id}`} className="flex items-center gap-1">
          <span className="font-medium">Est:</span>{card.estimatedHours}h
        </span>
      )}
      {card.actualHours && (
        <span data-testid={`${testId}-card-act-${card.id}`} className="flex items-center gap-1">
          <span className="font-medium">Act:</span>{card.actualHours}h
        </span>
      )}
    </div>
  )}
...
{card.dueDate && (
  ...
  {card.tags && card.tags.length > 0 && (
    <div className={kanbanClasses.card.tags}>
      ...
      {card.tags.length > 3 && (
        <span className={kanbanClasses.card.tag}>
          +{card.tags.length - 3}
        </span>
      )}
    </div>
        ...
        dragState, activeCard, readOnly, allowCardReorder, allowCardMoveBetweenColumns,
        handleCardDragStart, handleCardDragEnd, handleCardDrop, handleCardClick,
        handleDeleteCard, showCardValue, showProbability, testId
      ]);

      // Render default column
      const renderDefaultColumn = React.useCallback((column: KanbanColumn, index: number) => {
        const isDragging = dragState.sourceColumn?.id === column.id && dragState.type === 'column';
        const isOver = dragState.targetColumn?.id === column.id;
        ...
        const columnWeightedValue = column.cards.reduce((total, card) => total + calculateWeightedValue(card), 0);
        ...
        return (
          ...
          >
            ...
            <div className={kanbanClasses.column.header}>
              ...
              <div className="flex items-center gap-2">
                {showCardCounts && (
                  <span className={kanbanClasses.column.count}
            ...
            {!column.isCollapsed && (renderColumnFooter || showCardValue) && (
              <div className={kanbanClasses.column.footer}>
                {renderColumnFooter ? renderColumnFooter(column) : (
                  showCardValue && columnValue > 0 && (
                    <div className="text-xs font-medium text-gray-700 dark:text-gray-300">
                      Value: {column.currency || '$'}{columnValue.toLocaleString()}
                      {columnWeightedValue !== columnValue && (
                        <span className="text-gray-500 dark:text-gray-400 ml-1">
                          (Weighted: {column.currency || '$'}{columnWeightedValue.toLocaleString()})
                        </span>
                      )}
                    </div>
                  )
        ...
        dragState, readOnly, allowColumnReorder, minColumnWidth, onColumnClick,
        handleColumnDragStart, handleColumnDrop, handleColumnClick, handleDeleteColumn,
        handleAddCard, showCardCounts, showWipLimits, showCardValue, showColumnActions,
        renderCard, renderDefaultCard, renderColumnFooter, testId
      ]);
...
//   const totalCards = columns.reduce((total, col) => total + col.cards.length, 0);
  ...
  const averageDealSize = dealsWithValue.length > 0
    ? dealsWithValue.reduce((sum, card) => sum + (card.value || 0), 0) / dealsWithValue.length
    : 0;
  ...
  const winRate = dealsWithValue.length > 0
 ...
 const renderDefaultAnalytics = React.useCallback(() => {
  ...
  return (
    <div className="mb-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
      <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Pipeline Analytics</h3>
  ...
  ${analyticsData.totalValue.toLocaleString()}
      ...
      <div className="mt-4 text-sm text-gray-600 dark:text-gray-400 grid grid-cols-3 gap-4">
        ...
        <div>Active Deals: {columns.flatMap(col => col.cards).filter(card => card.probability && card.probability > 0).length}</div>
      </div>
    </div>
  );
    ...
    {renderColumnHeader
      ...
      : renderDefaultColumn(column, index)
            ...
            {dragState.draggingCard && dragState.position && (
              <div 
                className={cn(kanbanClasses.drag.ghost, getCardClasses(dragState.draggingCard.priority))}
                style={{
  ...
  React.forwardRef<HTMLDivElement, KanbanColumnProps>(
    ({ column, index, isDragging, isOver, className, style, ...props }, ref) => {
      ...
      return (
        <div
          ref={ref}
          className={getColumnClasses(column.color, isDragging, isOver, column.isCollapsed, className)}
  ...
  React.forwardRef<HTMLDivElement, KanbanCardProps>(
    ({ card, column, index, isDragging, isOver, className, style, ...props }, ref) => {
      ...
      return (
        <div
          ref={ref}
          className={getCardClasses(card.priority, isDragging, isOver, false, card.isArchived, className)}
