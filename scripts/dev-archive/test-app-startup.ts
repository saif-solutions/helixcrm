...
async function bootstrap() {
  ...
  try {
    ...
    try {
      ...
      console.log('⚠️  Soc2EvidenceService not in global context (expected if not exported globally)');
    }
...
bootstrap()
  .then(success => {
    console.log(success ? '✅ SUCCESS: Application can start with ComplianceModule' : '❌ FAILED: Application cannot start');
