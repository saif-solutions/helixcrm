...
import { AuditAction, AuditEntityType, AuditSeverity } from '../../shared/audit-log/audit-log.service';
import {
...
private async getUserEmail(userId: string): Promise<string> {
  ...
  async getDealAnalytics(query: DealAnalyticsQueryDto) {
    ...
    if (!this.permissionContext.hasPermission('analytics.read')) {
      throw new ForbiddenException('Insufficient permissions: analytics.read required');
    }
    ...
    try {
      ...
      if (this.useSummaryTables && !query.includeVelocity) {
        try {
          this.logger.debug('Attempting to use summary tables for deal analytics');
          const result = await this.analyticsSummaryRepository.getDealAnalyticsFromSummary(query);
          ...
          this.logger.warn('Summary table query failed, falling back to operational tables', {
            tenantId,
      ...
      // 3. OPERATIONAL PATH
      const result = await this.analyticsRepository.getDealAnalyticsFromOperational(query);
      ...
      this.logger.error(`getDealAnalytics failed: ${error.message}`, error.stack, {
        tenantId,
        userId,
      ...
      const performance = duration > 2000 ? 'slow' : duration > 1000 ? 'warning' : 'normal';
  ...
  async getRevenueAnalytics(query: RevenueAnalyticsQueryDto) {
    ...
    try {
      ...
      if (this.useSummaryTables) {
        try {
          this.logger.debug('Attempting to use summary tables for revenue analytics');
          const result = await this.analyticsSummaryRepository.getRevenueAnalyticsFromSummary(query);
      ...
      // 3. OPERATIONAL PATH
      const result = await this.analyticsRepository.getRevenueAnalyticsFromOperational(query);
      ...
      // 4. ERROR HANDLING
      this.logger.error(`getRevenueAnalytics failed: ${error.message}`, error.stack, {
        tenantId,
        userId,
  ...
  async getPipelineAnalytics(query: PipelineAnalyticsQueryDto) {
    ...
    try {
      ...
      if (this.useSummaryTables) {
        try {
          this.logger.debug('Attempting to use summary tables for pipeline analytics');
          
          // Note: The summary repository should handle this
          // If there's no dedicated summary method, we'll use operational
          const result = await this.analyticsRepository.getPipelineAnalyticsFromOperational(query);
      ...
      // 4. ERROR HANDLING
      this.logger.error(`getPipelineAnalytics failed: ${error.message}`, error.stack, {
        tenantId,
        userId,
  ...
  async getActivityAnalytics(query: ActivityAnalyticsQueryDto) {
    ...
    try {
      ...
      if (this.useSummaryTables) {
        try {
          this.logger.debug('Attempting to use summary tables for activity analytics');
          const result = await this.analyticsRepository.getActivityAnalyticsFromOperational(query);
      ...
      // 4. ERROR HANDLING
      this.logger.error(`getActivityAnalytics failed: ${error.message}`, error.stack, {
        tenantId,
        userId,
  ...
  async createAnalyticsExport(query: AnalyticsExportQueryDto) {
    ...
    if (!this.permissionContext.hasPermission('analytics.export')) {
      throw new ForbiddenException('Insufficient permissions: analytics.export required');
    }
    ...
    try {
      ...
      const availableExports = await this.analyticsRepository.getAvailableExports(query, tenantId, userId);
...
await this.auditLogService.logEvent({
  ...
  organizationId: tenantId
      ...
      // 6. ERROR HANDLING
      this.logger.error(`createAnalyticsExport failed: ${error.message}`, error.stack, {
        tenantId,
        userId,
  ...
  async getExportStatus(jobId: string) {
    ...
    try {
      ...
      // 4. ERROR HANDLING
      this.logger.error(`getExportStatus failed: ${error.message}`, error.stack, {
        tenantId,
        userId,
...
await this.auditLogService.logEvent({
  ...
  organizationId: tenantId
      ...
      // 6. ERROR HANDLING
      this.logger.error(`downloadExport failed: ${error.message}`, error.stack, {
        tenantId,
        userId,
  ...
  async getAvailableExports(query: AnalyticsExportQueryDto) {
    ...
    try {
      ...
      const exports = await this.analyticsRepository.getAvailableExports(query, tenantId, userId);
      
      return exports;
      
    } catch (error: any) {
      // 4. ERROR HANDLING
      this.logger.error(`getAvailableExports failed: ${error.message}`, error.stack, {
        tenantId,
        userId,
  ...
  private async getDealAnalyticsFromSummary(organizationId: string, query: any): Promise<any> {
    this.logger.warn('DEPRECATED: getDealAnalyticsFromSummary called - use repository instead');
    throw new Error('Method deprecated - use analyticsSummaryRepository.getDealAnalyticsFromSummary');
  }

  private async getDealAnalyticsFromOperational(organizationId: string, query: any): Promise<any> {
    this.logger.warn('DEPRECATED: getDealAnalyticsFromOperational called - use repository instead');
    throw new Error('Method deprecated - use analyticsRepository.getDealAnalyticsFromOperational');
  }

  private async getRevenueAnalyticsFromSummary(organizationId: string, query: any): Promise<any> {
    this.logger.warn('DEPRECATED: getRevenueAnalyticsFromSummary called - use repository instead');
    throw new Error('Method deprecated - use analyticsSummaryRepository.getRevenueAnalyticsFromSummary');
  }

  private buildDealWhereClause(organizationId: string, query: any) {
    this.logger.warn('DEPRECATED: buildDealWhereClause called - repository handles filtering');
    return {};
  }

  private async getPipelineDataFromSummary(organizationId: string, query: any): Promise<any> {
    this.logger.warn('DEPRECATED: getPipelineDataFromSummary called - use repository instead');
    throw new Error('Method deprecated - repository handles this');
  }

  private async getActivityAnalyticsFromSummary(organizationId: string, query: any): Promise<any> {
    this.logger.warn('DEPRECATED: getActivityAnalyticsFromSummary called - use repository instead');
    throw new Error('Method deprecated - use analyticsRepository.getActivityAnalyticsFromOperational');
  }

  private async getActivityAnalyticsFromOperational(organizationId: string, query: any): Promise<any> {
    this.logger.warn('DEPRECATED: getActivityAnalyticsFromOperational called - use repository instead');
    throw new Error('Method deprecated - use analyticsRepository.getActivityAnalyticsFromOperational');
  }

  // Legacy methods for backward compatibility
  async queueExportJob(organizationId: string, userId: string, query: any): Promise<any> {
    this.logger.warn('DEPRECATED: queueExportJob called with organizationId parameter');
  ...
  async getExportData(token: string, organizationId: string, userId: string): Promise<any> {
    this.logger.warn('DEPRECATED: getExportData called with organizationId parameter');
    // Extract jobId from token
    const jobId = token.replace('token-', '');
    const result = await this.downloadExport(jobId, token);
    return {
      format: result.contentType.includes('csv') ? ExportFormat.CSV : ExportFormat.JSON,
      exportId: jobId,
      data: result.data
    };
  }
