...
export class LeadsService {
  ...
  async create(createLeadDto: CreateLeadDto, userId: string) {
    ...
    try {
      ...
      this.logger.log(`Lead.create completed in ${duration}ms`, {
        ...
        method: 'create',
        tenantId: this.tenantContext.getTenantId(),
        performance: duration > 1000 ? 'slow' : duration > 500 ? 'warning' : 'normal'
      });
    }
  }
  ...
  async findAll(params: FindAllOptions) {
    ...
    if (!this.permissionContext.hasPermission('leads.read')) {
      throw new ForbiddenException('Insufficient permissions: leads.read required');
    }
  ...
  async findOne(id: string) {
    ...
    try {
      ...
      this.logger.error(`Failed to fetch lead ${id}: ${error.message}`, error.stack, {
        leadId: id,
        tenantId: this.tenantContext.getTenantId(),
      });
      throw error;
  ...
  async remove(id: string, userId: string) {
    ...
    if (!this.permissionContext.hasPermission('leads.delete')) {
      throw new ForbiddenException('Insufficient permissions: leads.delete required');
    }
  ...
  async getStats() {
    ...
    try {
      ...
      this.logger.error(`Failed to fetch lead stats: ${error.message}`, error.stack, {
        tenantId: this.tenantContext.getTenantId(),
      });
      throw error;
    } finally {
