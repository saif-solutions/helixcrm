...
find apps/api/src -name "*.ts" -type f -exec grep -l "AsyncLocalStorage" {} \; | grep -v node_modules

echo ""
echo "Repositories extending TenantAwareRepository:"
find apps/api/src -name "*.ts" -type f -exec grep -l "extends TenantAwareRepository" {} \; | grep -v node_modules
