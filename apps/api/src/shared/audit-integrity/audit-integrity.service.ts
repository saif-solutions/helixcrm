...
export class AuditIntegrityService {
   ...
   */
  private generateHash(event: AuditEvent, previousHash: string, config: IntegrityConfig = this.defaultConfig): string {
  ...
  async appendEvent(event: AuditEvent): Promise<string> {
    ...
    try {
      ...
      this.logger.debug(`Appended audit event to integrity chain. Block: ${nextIndex}, Hash: ${eventHash.substring(0, 16)}...`);
      
      return eventHash;
    } catch (error) {
      this.logger.error(`Failed to append audit event to integrity chain: ${error.message}`, error.stack);
  ...
  async verifyChain(): Promise<VerificationResult> {
    ...
    try {
      ...
      for (const event of allEvents) {
        ...
        const calculatedHash = this.generateHash(auditEvent, previousHash, config);
        ...
        if (event.eventHash !== calculatedHash) {
          ...
          this.logger.error(`Audit chain integrity violation at block ${event.blockIndex}`);
      ...
      if (isChainValid) {
        this.logger.log(`Audit chain verification successful. Total events: ${allEvents.length}`);
      } else {
        this.logger.error(`Audit chain verification FAILED at block ${brokenAtIndex}`);
      }
      
      return result;
    } catch (error) {
      this.logger.error(`Audit chain verification error: ${error.message}`, error.stack);
  ...
  async getChainStats() {
    try {
      const [totalEvents, lastVerification, firstBlock, lastBlock] = await Promise.all([
      ...
      return {
        ...
        chainLengthDays: firstBlock && lastBlock 
          ? Math.ceil((lastBlock.timestamp.getTime() - firstBlock.timestamp.getTime()) / (1000 * 60 * 60 * 24))
          : 0,
  ...
  async repairChain(): Promise<{ repaired: boolean; message: string }> {
    this.logger.warn('Manual chain repair initiated - this should only be used in emergencies');
    ...
    try {
      ...
      this.logger.error(`Chain repair needed at block ${verification.brokenAtIndex}`);
