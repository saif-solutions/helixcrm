...
export class AnalyticsSummaryRepository extends TenantAwareRepository {
  ...
  async getDealAnalyticsFromSummary(query: any): Promise<any> {
    try {
      ...
      const summaryExists = await this.checkSummaryTableExists('deal_summary_daily');
      if (!summaryExists) {
...
where: {
  ...
  date: {
    ...
    ...(query.endDate ? { lte: query.endDate } : {})
      ...
      const stageMetrics = stageSummary.reduce((acc, item) => {
      ...
      return {
        ...
        summary: {
          ...
          activeDeals: stageMetrics.find(s => s.stage === 'ACTIVE')?.count || 0,
        },
        source: 'summary',
      };
    } catch (error: any) {
      this.logger.warn(`Failed to get deal analytics from summary: ${error.message}`);
      throw error;
    }
  }
  ...
  async getRevenueAnalyticsFromSummary(query: any): Promise<any> {
    try {
      ...
      // Check if summary tables exist
      const summaryExists = await this.checkSummaryTableExists('revenue_summary_daily');
      if (!summaryExists) {
        throw new Error('Summary tables not available');
      }
  ...
  date: {
    ...
    ...(query.endDate ? { lte: query.endDate } : {})
      ...
      const forecastSummary = await this.prisma.dealForecastSummaryDaily.findMany({
       where: {
  organizationId: tenantId,
  date: {
    ...(query.startDate ? { gte: query.startDate } : {}),
    ...(query.endDate ? { lte: query.endDate } : {})
     ...
     const forecastRevenue = forecastSummary.reduce((sum, item) => sum + Number(item.forecastRevenue), 0);

      return {
        forecastRevenue,
        revenueOverTime,
        summary: {
          totalRevenue: revenueOverTime.reduce((sum, item) => sum + Number(item.revenue), 0),
          forecastedRevenue: forecastRevenue,
          averageDealSize: revenueOverTime.length > 0 
            ? revenueOverTime.reduce((sum, item) => sum + Number(item.revenue), 0) / revenueOverTime.length
            : 0,
        },
        source: 'summary',
      };
    } catch (error: any) {
      this.logger.warn(`Failed to get revenue analytics from summary: ${error.message}`);
      throw error;
    }
  }
